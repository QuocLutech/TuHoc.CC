    ...
    ***Ở ảnh này có thể thấy : + App.vue chứa ProductList : 	<script setup>
    ...
    #4.4 : Slot
    	...
    	// Giả sử có component : 
			...
			</template> //<slot> </slot> là vị trí chờ nội dung từ component cha truyền vào

			giả sử component cha App.vue : 
				<template>
  					<card>
    					<h2>hello world</h2>
				...
				-Vue sẽ render tương đương
				-cùng một component card nhưng nội dung bên trong khác nhau
				-bạn cũng có thể cho slot nội dung mặc định :
						...
						<template>
							...
							nhưng nếu cha không truyền đi thông tin gì thì nội dung mặc định sẽ được thay bằng nội  dung mặc định .
	
	-Component tái sử dụng : Component tái sử dụng (Reusable Component) là việc bạn tạo một component một lần, sau đó có thể sử dụng nó nhiều nơi trong ứng dụng.

##5 : Reactivity(tính phản ứng) là cơ chế giúp Vue tự động cập nhật giao diện khi dữ liệu thay đổi.
		Dữ liệu thay đổi → Vue phát hiện → giao diện tự cập nhật.
    ...
    #5.1 : ref() có những trường hợp chúng ta cần truy cập trực tiếp vào các phần tử DOM cơ bản. Để đạt được điều này, chúng ta có thể sử dụng ref.
			...
			ví dụ : const name = ref('Quốc')
			---> trong javaScript name.value = 'Nguyễn Quốc'
			---> Vue phát hiện thay đổi và cập nhật <h2>{{name}}<h2> ==> Nguyễn Quốc
			https://github.com/user-attachments/assets/531f5815-046b-4178-a2e1-b73acaec8f5b
    ...
    #5.4 : watch()
		...
		8c-8d70-4926-8ee0-ed7681bd579f
		-Cấu trúc watch : 
					watch(
  							dữ_liệu_cần_theo_dõi,
  							(giá_trị_mới, giá_trị_cũ) => {
    						// hành động muốn thực hiện
  														 }
						 )
		- VÍ DỤ : watch(count, (newValue, oldValue) => {
		...
		- watch() và computed() khác nhau , computed() dùng khi muốn tạo ra một giá trị mới còn watch() dùng khi bạn muốn làm một hành động dữ liệu thay đổi . Ví dụ : computed() thì tính tổng tiền còn watch thì tính số lượng tiền thay đổi
